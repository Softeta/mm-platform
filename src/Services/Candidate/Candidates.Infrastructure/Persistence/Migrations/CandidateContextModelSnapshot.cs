// <auto-generated />
using System;
using Candidates.Infrastructure.Persistence;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using NetTopologySuite.Geometries;

#nullable disable

namespace Candidates.Infrastructure.Persistence.Migrations
{
    [DbContext(typeof(CandidateContext))]
    partial class CandidateContextModelSnapshot : ModelSnapshot
    {
        protected override void BuildModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.2")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Bio")
                        .HasMaxLength(4000)
                        .HasColumnType("nvarchar(4000)");

                    b.Property<DateTimeOffset?>("BirthDate")
                        .HasColumnType("datetimeoffset");

                    b.Property<long?>("CandidateOldPlatformId")
                        .HasColumnType("bigint");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid?>("ExternalId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("FirstName")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<bool>("IsHired")
                        .HasColumnType("bit");

                    b.Property<bool>("IsShortListed")
                        .HasColumnType("bit");

                    b.Property<string>("LastName")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("LinkedInUrl")
                        .HasMaxLength(2000)
                        .HasColumnType("nvarchar(2000)");

                    b.Property<bool>("OpenForOpportunities")
                        .HasColumnType("bit");

                    b.Property<string>("PersonalWebsiteUrl")
                        .HasMaxLength(2000)
                        .HasColumnType("nvarchar(2000)");

                    b.Property<DateTimeOffset?>("RejectedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("SystemLanguage")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<int?>("YearsOfExperience")
                        .HasColumnType("int");

                    b.HasKey("Id");

                    b.ToTable("Candidates", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateActivityStatus", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ActivityStatus")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.ToTable("CandidateActivityStatuses", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateCourse", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("Description")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("IssuingOrganization")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.ToTable("CandidateCourses", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateDesiredSkill", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid>("SkillId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.HasIndex("CandidateId", "SkillId")
                        .IsUnique();

                    b.ToTable("CandidateDesiredSkills", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateEducation", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("Degree")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("FieldOfStudy")
                        .IsRequired()
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("IsStillStudying")
                        .HasColumnType("bit");

                    b.Property<string>("SchoolName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("StudiesDescription")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.ToTable("CandidateEducations", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateHobby", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid>("HobbyId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.HasIndex("CandidateId", "HobbyId")
                        .IsUnique();

                    b.ToTable("CandidateHobbies", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateIndustry", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid>("IndustryId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.HasIndex("CandidateId", "IndustryId")
                        .IsUnique();

                    b.ToTable("CandidateIndustries", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateLanguage", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.ToTable("CandidateLanguages", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateShortListedJob", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid>("JobId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.ToTable("CandidateShortListedJobs");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateSkill", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid>("SkillId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.HasIndex("CandidateId", "SkillId")
                        .IsUnique();

                    b.ToTable("CandidateSkills", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateWorkExperience", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CompanyName")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<bool>("IsCurrentJob")
                        .HasColumnType("bit");

                    b.Property<string>("JobDescription")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.ToTable("CandidateWorkExperiences", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateWorkExperienceSkill", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateWorkExperienceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid>("SkillId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("CandidateWorkExperienceId");

                    b.HasIndex("CandidateWorkExperienceId", "SkillId")
                        .IsUnique();

                    b.ToTable("CandidateWorkExperienceSkills", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateJobsAggregate.CandidateJobs", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<bool>("IsHired")
                        .HasColumnType("bit");

                    b.Property<bool>("IsShortlisted")
                        .HasColumnType("bit");

                    b.HasKey("Id");

                    b.ToTable("CandidateJobs", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateJobsAggregate.Entities.CandidateAppliedInJob", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<DateTimeOffset?>("DeadlineDate")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("Freelance")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<bool>("IsJobArchived")
                        .HasColumnType("bit");

                    b.Property<Guid>("JobId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("JobStage")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("Permanent")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<DateTimeOffset?>("StartDate")
                        .HasColumnType("datetimeoffset");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.HasIndex("CandidateId", "JobId")
                        .IsUnique();

                    b.ToTable("CandidateAppliedInJobs", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateJobsAggregate.Entities.CandidateArchivedInJob", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CoverLetter")
                        .HasMaxLength(4000)
                        .HasColumnType("nvarchar(4000)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<DateTimeOffset?>("DeadlineDate")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("Freelance")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<bool>("HasApplied")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("InvitedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<bool>("IsJobArchived")
                        .HasColumnType("bit");

                    b.Property<Guid>("JobId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("JobStage")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("Permanent")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("Stage")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<DateTimeOffset?>("StartDate")
                        .HasColumnType("datetimeoffset");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.HasIndex("CandidateId", "JobId")
                        .IsUnique();

                    b.ToTable("CandidateArchivedInJobs", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateJobsAggregate.Entities.CandidateSelectedInJob", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("CandidateId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CoverLetter")
                        .HasMaxLength(4000)
                        .HasColumnType("nvarchar(4000)");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<DateTimeOffset?>("DeadlineDate")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("Freelance")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<bool>("HasApplied")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("InvitedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<bool>("IsJobArchived")
                        .HasColumnType("bit");

                    b.Property<Guid>("JobId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("JobStage")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("Permanent")
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<string>("Stage")
                        .IsRequired()
                        .HasMaxLength(64)
                        .HasColumnType("nvarchar(64)");

                    b.Property<DateTimeOffset?>("StartDate")
                        .HasColumnType("datetimeoffset");

                    b.HasKey("Id");

                    b.HasIndex("CandidateId");

                    b.HasIndex("CandidateId", "JobId")
                        .IsUnique();

                    b.ToTable("CandidateSelectedInJobs", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateTestsAggregate.CandidateTests", b =>
                {
                    b.Property<Guid>("Id")
                        .HasColumnType("uniqueidentifier");

                    b.Property<long?>("CandidateOldPlatformId")
                        .HasColumnType("bigint");

                    b.Property<DateTimeOffset>("CreatedAt")
                        .HasColumnType("datetimeoffset");

                    b.Property<Guid>("ExternalId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id");

                    b.HasIndex("CandidateOldPlatformId");

                    b.HasIndex("ExternalId");

                    b.ToTable("CandidateTests", "candidate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", b =>
                {
                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Position", "CurrentPosition", b1 =>
                        {
                            b1.Property<Guid>("CandidateId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Code")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.HasKey("CandidateId");

                            b1.ToTable("Candidates", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateId");

                            b1.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Tag", "AliasTo", b2 =>
                                {
                                    b2.Property<Guid>("PositionCandidateId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<string>("Code")
                                        .IsRequired()
                                        .HasMaxLength(64)
                                        .HasColumnType("nvarchar(64)");

                                    b2.Property<Guid>("Id")
                                        .HasColumnType("uniqueidentifier");

                                    b2.HasKey("PositionCandidateId");

                                    b2.ToTable("Candidates", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("PositionCandidateId");
                                });

                            b1.Navigation("AliasTo");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Document", "CurriculumVitae", b1 =>
                        {
                            b1.Property<Guid>("CandidateId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("FileName")
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("Uri")
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateId");

                            b1.ToTable("Candidates", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.LegalInformationAgreement", "MarketingAcknowledgement", b1 =>
                        {
                            b1.Property<Guid>("CandidateId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<bool>("Agreed")
                                .HasColumnType("bit");

                            b1.Property<DateTimeOffset>("ModifiedAt")
                                .HasColumnType("datetimeoffset");

                            b1.HasKey("CandidateId");

                            b1.ToTable("Candidates", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.LegalInformationAgreement", "TermsAndConditions", b1 =>
                        {
                            b1.Property<Guid>("CandidateId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<bool>("Agreed")
                                .HasColumnType("bit");

                            b1.Property<DateTimeOffset>("ModifiedAt")
                                .HasColumnType("datetimeoffset");

                            b1.HasKey("CandidateId");

                            b1.ToTable("Candidates", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Document", "Video", b1 =>
                        {
                            b1.Property<Guid>("CandidateId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("FileName")
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("Uri")
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateId");

                            b1.ToTable("Candidates", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateId");
                        });

                    b.OwnsOne("Candidates.Domain.Aggregates.CandidateAggregate.ValueObjects.Note", "Note", b1 =>
                        {
                            b1.Property<Guid>("CandidateId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<DateTimeOffset?>("EndDate")
                                .HasColumnType("datetimeoffset");

                            b1.Property<string>("Value")
                                .IsRequired()
                                .HasMaxLength(250)
                                .HasColumnType("nvarchar(250)");

                            b1.HasKey("CandidateId");

                            b1.ToTable("Candidates", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateId");
                        });

                    b.OwnsOne("Candidates.Domain.Aggregates.CandidateAggregate.ValueObjects.Terms", "Terms", b1 =>
                        {
                            b1.Property<Guid>("CandidateId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Currency")
                                .HasColumnType("nvarchar(max)");

                            b1.HasKey("CandidateId");

                            b1.ToTable("Candidates", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateId");

                            b1.OwnsOne("Candidates.Domain.Aggregates.CandidateAggregate.ValueObjects.Freelance", "Freelance", b2 =>
                                {
                                    b2.Property<Guid>("TermsCandidateId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<decimal?>("HourlySalary")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal?>("MonthlySalary")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<string>("WorkType")
                                        .IsRequired()
                                        .HasMaxLength(64)
                                        .HasColumnType("nvarchar(64)");

                                    b2.HasKey("TermsCandidateId");

                                    b2.ToTable("Candidates", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("TermsCandidateId");
                                });

                            b1.OwnsOne("Candidates.Domain.Aggregates.CandidateAggregate.ValueObjects.FullTimeWorkingHours", "FulTimeWorkingHours", b2 =>
                                {
                                    b2.Property<Guid>("TermsCandidateId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<string>("Type")
                                        .IsRequired()
                                        .HasMaxLength(64)
                                        .HasColumnType("nvarchar(64)");

                                    b2.HasKey("TermsCandidateId");

                                    b2.ToTable("Candidates", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("TermsCandidateId");
                                });

                            b1.OwnsOne("Candidates.Domain.Aggregates.CandidateAggregate.ValueObjects.JobFormats", "Formats", b2 =>
                                {
                                    b2.Property<Guid>("TermsCandidateId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<bool>("IsHybrid")
                                        .HasColumnType("bit");

                                    b2.Property<bool>("IsOnSite")
                                        .HasColumnType("bit");

                                    b2.Property<bool>("IsRemote")
                                        .HasColumnType("bit");

                                    b2.HasKey("TermsCandidateId");

                                    b2.ToTable("Candidates", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("TermsCandidateId");
                                });

                            b1.OwnsOne("Candidates.Domain.Aggregates.CandidateAggregate.ValueObjects.PartTimeWorkingHours", "PartTimeWorkingHours", b2 =>
                                {
                                    b2.Property<Guid>("TermsCandidateId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<string>("Type")
                                        .IsRequired()
                                        .HasMaxLength(64)
                                        .HasColumnType("nvarchar(64)");

                                    b2.Property<int?>("Weekly")
                                        .HasColumnType("int");

                                    b2.HasKey("TermsCandidateId");

                                    b2.ToTable("Candidates", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("TermsCandidateId");
                                });

                            b1.OwnsOne("Candidates.Domain.Aggregates.CandidateAggregate.ValueObjects.Permanent", "Permanent", b2 =>
                                {
                                    b2.Property<Guid>("TermsCandidateId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<decimal?>("MonthlySalary")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<string>("WorkType")
                                        .IsRequired()
                                        .HasMaxLength(64)
                                        .HasColumnType("nvarchar(64)");

                                    b2.HasKey("TermsCandidateId");

                                    b2.ToTable("Candidates", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("TermsCandidateId");
                                });

                            b1.OwnsOne("Candidates.Domain.Aggregates.CandidateAggregate.ValueObjects.ProjectWorkingHours", "ProjectWorkingHours", b2 =>
                                {
                                    b2.Property<Guid>("TermsCandidateId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<string>("Type")
                                        .IsRequired()
                                        .HasMaxLength(64)
                                        .HasColumnType("nvarchar(64)");

                                    b2.HasKey("TermsCandidateId");

                                    b2.ToTable("Candidates", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("TermsCandidateId");
                                });

                            b1.OwnsOne("Domain.Seedwork.Shared.ValueObjects.DateRange", "Availability", b2 =>
                                {
                                    b2.Property<Guid>("TermsCandidateId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<DateTimeOffset>("From")
                                        .HasColumnType("datetimeoffset");

                                    b2.Property<DateTimeOffset?>("To")
                                        .HasColumnType("datetimeoffset");

                                    b2.HasKey("TermsCandidateId");

                                    b2.ToTable("Candidates", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("TermsCandidateId");
                                });

                            b1.Navigation("Availability");

                            b1.Navigation("Formats");

                            b1.Navigation("Freelance");

                            b1.Navigation("FulTimeWorkingHours");

                            b1.Navigation("PartTimeWorkingHours");

                            b1.Navigation("Permanent");

                            b1.Navigation("ProjectWorkingHours");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Address", "Address", b1 =>
                        {
                            b1.Property<Guid>("CandidateId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("AddressLine")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("City")
                                .HasMaxLength(64)
                                .HasColumnType("nvarchar(64)");

                            b1.Property<string>("Country")
                                .HasMaxLength(64)
                                .HasColumnType("nvarchar(64)");

                            b1.Property<string>("Location")
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("PostalCode")
                                .HasMaxLength(64)
                                .HasColumnType("nvarchar(64)");

                            b1.HasKey("CandidateId");

                            b1.ToTable("Candidates", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateId");

                            b1.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Coordinates", "Coordinates", b2 =>
                                {
                                    b2.Property<Guid>("AddressCandidateId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<double>("Latitude")
                                        .HasColumnType("float");

                                    b2.Property<double>("Longitude")
                                        .HasColumnType("float");

                                    b2.Property<Point>("Point")
                                        .IsRequired()
                                        .HasColumnType("geography");

                                    b2.HasKey("AddressCandidateId");

                                    b2.ToTable("Candidates", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("AddressCandidateId");
                                });

                            b1.Navigation("Coordinates");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Email", "Email", b1 =>
                        {
                            b1.Property<Guid>("CandidateId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Address")
                                .IsRequired()
                                .HasMaxLength(64)
                                .HasColumnType("nvarchar(64)");

                            b1.Property<bool>("IsVerified")
                                .HasColumnType("bit");

                            b1.Property<Guid?>("VerificationKey")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<DateTimeOffset?>("VerificationRequestedAt")
                                .HasColumnType("datetimeoffset");

                            b1.Property<DateTimeOffset?>("VerifiedAt")
                                .HasColumnType("datetimeoffset");

                            b1.HasKey("CandidateId");

                            b1.ToTable("Candidates", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Image", "Picture", b1 =>
                        {
                            b1.Property<Guid>("CandidateId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("OriginalUri")
                                .IsRequired()
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.Property<string>("ThumbnailUri")
                                .IsRequired()
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateId");

                            b1.ToTable("Candidates", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Phone", "Phone", b1 =>
                        {
                            b1.Property<Guid>("CandidateId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("CountryCode")
                                .HasMaxLength(4)
                                .HasColumnType("nvarchar(4)");

                            b1.Property<string>("Number")
                                .HasMaxLength(28)
                                .HasColumnType("nvarchar(28)");

                            b1.Property<string>("PhoneNumber")
                                .HasMaxLength(32)
                                .HasColumnType("nvarchar(32)");

                            b1.HasKey("CandidateId");

                            b1.ToTable("Candidates", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateId");
                        });

                    b.Navigation("Address");

                    b.Navigation("CurrentPosition");

                    b.Navigation("CurriculumVitae");

                    b.Navigation("Email");

                    b.Navigation("MarketingAcknowledgement");

                    b.Navigation("Note");

                    b.Navigation("Phone");

                    b.Navigation("Picture");

                    b.Navigation("Terms")
                        .IsRequired();

                    b.Navigation("TermsAndConditions");

                    b.Navigation("Video");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateActivityStatus", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", null)
                        .WithMany("ActivityStatuses")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateCourse", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", null)
                        .WithMany("Courses")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Document", "Certificate", b1 =>
                        {
                            b1.Property<Guid>("CandidateCourseId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("FileName")
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("Uri")
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateCourseId");

                            b1.ToTable("CandidateCourses", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateCourseId");
                        });

                    b.Navigation("Certificate");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateDesiredSkill", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", null)
                        .WithMany("DesiredSkills")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Tag", "AliasTo", b1 =>
                        {
                            b1.Property<Guid>("CandidateDesiredSkillId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Code")
                                .IsRequired()
                                .HasMaxLength(64)
                                .HasColumnType("nvarchar(64)");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.HasKey("CandidateDesiredSkillId");

                            b1.ToTable("CandidateDesiredSkills", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateDesiredSkillId");
                        });

                    b.Navigation("AliasTo");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateEducation", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", null)
                        .WithMany("Educations")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Document", "Certificate", b1 =>
                        {
                            b1.Property<Guid>("CandidateEducationId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("FileName")
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("Uri")
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateEducationId");

                            b1.ToTable("CandidateEducations", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateEducationId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.DateRange", "Period", b1 =>
                        {
                            b1.Property<Guid>("CandidateEducationId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<DateTimeOffset>("From")
                                .HasColumnType("datetimeoffset");

                            b1.Property<DateTimeOffset?>("To")
                                .HasColumnType("datetimeoffset");

                            b1.HasKey("CandidateEducationId");

                            b1.ToTable("CandidateEducations", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateEducationId");
                        });

                    b.Navigation("Certificate");

                    b.Navigation("Period")
                        .IsRequired();
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateHobby", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", null)
                        .WithMany("Hobbies")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateIndustry", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", null)
                        .WithMany("Industries")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateLanguage", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", null)
                        .WithMany("Languages")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Language", "Language", b1 =>
                        {
                            b1.Property<Guid>("CandidateLanguageId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Code")
                                .IsRequired()
                                .HasMaxLength(64)
                                .HasColumnType("nvarchar(64)");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Name")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.HasKey("CandidateLanguageId");

                            b1.ToTable("CandidateLanguages", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateLanguageId");
                        });

                    b.Navigation("Language")
                        .IsRequired();
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateSkill", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", null)
                        .WithMany("Skills")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Tag", "AliasTo", b1 =>
                        {
                            b1.Property<Guid>("CandidateSkillId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Code")
                                .IsRequired()
                                .HasMaxLength(64)
                                .HasColumnType("nvarchar(64)");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.HasKey("CandidateSkillId");

                            b1.ToTable("CandidateSkills", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateSkillId");
                        });

                    b.Navigation("AliasTo");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateWorkExperience", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", null)
                        .WithMany("WorkExperiences")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.DateRange", "Period", b1 =>
                        {
                            b1.Property<Guid>("CandidateWorkExperienceId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<DateTimeOffset>("From")
                                .HasColumnType("datetimeoffset");

                            b1.Property<DateTimeOffset?>("To")
                                .HasColumnType("datetimeoffset");

                            b1.HasKey("CandidateWorkExperienceId");

                            b1.ToTable("CandidateWorkExperiences", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateWorkExperienceId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Position", "Position", b1 =>
                        {
                            b1.Property<Guid>("CandidateWorkExperienceId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Code")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.HasKey("CandidateWorkExperienceId");

                            b1.ToTable("CandidateWorkExperiences", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateWorkExperienceId");

                            b1.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Tag", "AliasTo", b2 =>
                                {
                                    b2.Property<Guid>("PositionCandidateWorkExperienceId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<string>("Code")
                                        .IsRequired()
                                        .HasMaxLength(64)
                                        .HasColumnType("nvarchar(64)");

                                    b2.Property<Guid>("Id")
                                        .HasColumnType("uniqueidentifier");

                                    b2.HasKey("PositionCandidateWorkExperienceId");

                                    b2.ToTable("CandidateWorkExperiences", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("PositionCandidateWorkExperienceId");
                                });

                            b1.Navigation("AliasTo");
                        });

                    b.Navigation("Period")
                        .IsRequired();

                    b.Navigation("Position")
                        .IsRequired();
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateWorkExperienceSkill", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateWorkExperience", null)
                        .WithMany("Skills")
                        .HasForeignKey("CandidateWorkExperienceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Tag", "AliasTo", b1 =>
                        {
                            b1.Property<Guid>("CandidateWorkExperienceSkillId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Code")
                                .IsRequired()
                                .HasMaxLength(64)
                                .HasColumnType("nvarchar(64)");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.HasKey("CandidateWorkExperienceSkillId");

                            b1.ToTable("CandidateWorkExperienceSkills", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateWorkExperienceSkillId");
                        });

                    b.Navigation("AliasTo");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateJobsAggregate.Entities.CandidateAppliedInJob", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateJobsAggregate.CandidateJobs", null)
                        .WithMany("AppliedInJobs")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("Candidates.Domain.Aggregates.CandidateJobsAggregate.ValueObjects.Company", "Company", b1 =>
                        {
                            b1.Property<Guid>("CandidateAppliedInJobId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("LogoUri")
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.Property<string>("Name")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.HasKey("CandidateAppliedInJobId");

                            b1.ToTable("CandidateAppliedInJobs", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateAppliedInJobId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Position", "Position", b1 =>
                        {
                            b1.Property<Guid>("CandidateAppliedInJobId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Code")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.HasKey("CandidateAppliedInJobId");

                            b1.ToTable("CandidateAppliedInJobs", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateAppliedInJobId");

                            b1.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Tag", "AliasTo", b2 =>
                                {
                                    b2.Property<Guid>("PositionCandidateAppliedInJobId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<string>("Code")
                                        .IsRequired()
                                        .HasMaxLength(64)
                                        .HasColumnType("nvarchar(64)");

                                    b2.Property<Guid>("Id")
                                        .HasColumnType("uniqueidentifier");

                                    b2.HasKey("PositionCandidateAppliedInJobId");

                                    b2.ToTable("CandidateAppliedInJobs", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("PositionCandidateAppliedInJobId");
                                });

                            b1.Navigation("AliasTo");
                        });

                    b.Navigation("Company")
                        .IsRequired();

                    b.Navigation("Position")
                        .IsRequired();
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateJobsAggregate.Entities.CandidateArchivedInJob", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateJobsAggregate.CandidateJobs", null)
                        .WithMany("ArchivedInJobs")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("Candidates.Domain.Aggregates.CandidateJobsAggregate.ValueObjects.Company", "Company", b1 =>
                        {
                            b1.Property<Guid>("CandidateArchivedInJobId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("LogoUri")
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.Property<string>("Name")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.HasKey("CandidateArchivedInJobId");

                            b1.ToTable("CandidateArchivedInJobs", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateArchivedInJobId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Document", "MotivationVideo", b1 =>
                        {
                            b1.Property<Guid>("CandidateArchivedInJobId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("FileName")
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("Uri")
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateArchivedInJobId");

                            b1.ToTable("CandidateArchivedInJobs", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateArchivedInJobId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Position", "Position", b1 =>
                        {
                            b1.Property<Guid>("CandidateArchivedInJobId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Code")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.HasKey("CandidateArchivedInJobId");

                            b1.ToTable("CandidateArchivedInJobs", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateArchivedInJobId");

                            b1.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Tag", "AliasTo", b2 =>
                                {
                                    b2.Property<Guid>("PositionCandidateArchivedInJobId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<string>("Code")
                                        .IsRequired()
                                        .HasMaxLength(64)
                                        .HasColumnType("nvarchar(64)");

                                    b2.Property<Guid>("Id")
                                        .HasColumnType("uniqueidentifier");

                                    b2.HasKey("PositionCandidateArchivedInJobId");

                                    b2.ToTable("CandidateArchivedInJobs", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("PositionCandidateArchivedInJobId");
                                });

                            b1.Navigation("AliasTo");
                        });

                    b.Navigation("Company")
                        .IsRequired();

                    b.Navigation("MotivationVideo");

                    b.Navigation("Position")
                        .IsRequired();
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateJobsAggregate.Entities.CandidateSelectedInJob", b =>
                {
                    b.HasOne("Candidates.Domain.Aggregates.CandidateJobsAggregate.CandidateJobs", null)
                        .WithMany("SelectedInJobs")
                        .HasForeignKey("CandidateId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.OwnsOne("Candidates.Domain.Aggregates.CandidateJobsAggregate.ValueObjects.Company", "Company", b1 =>
                        {
                            b1.Property<Guid>("CandidateSelectedInJobId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("LogoUri")
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.Property<string>("Name")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.HasKey("CandidateSelectedInJobId");

                            b1.ToTable("CandidateSelectedInJobs", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateSelectedInJobId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Document", "MotivationVideo", b1 =>
                        {
                            b1.Property<Guid>("CandidateSelectedInJobId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("FileName")
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("Uri")
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateSelectedInJobId");

                            b1.ToTable("CandidateSelectedInJobs", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateSelectedInJobId");
                        });

                    b.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Position", "Position", b1 =>
                        {
                            b1.Property<Guid>("CandidateSelectedInJobId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("Code")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<Guid>("Id")
                                .HasColumnType("uniqueidentifier");

                            b1.HasKey("CandidateSelectedInJobId");

                            b1.ToTable("CandidateSelectedInJobs", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateSelectedInJobId");

                            b1.OwnsOne("Domain.Seedwork.Shared.ValueObjects.Tag", "AliasTo", b2 =>
                                {
                                    b2.Property<Guid>("PositionCandidateSelectedInJobId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<string>("Code")
                                        .IsRequired()
                                        .HasMaxLength(64)
                                        .HasColumnType("nvarchar(64)");

                                    b2.Property<Guid>("Id")
                                        .HasColumnType("uniqueidentifier");

                                    b2.HasKey("PositionCandidateSelectedInJobId");

                                    b2.ToTable("CandidateSelectedInJobs", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("PositionCandidateSelectedInJobId");
                                });

                            b1.Navigation("AliasTo");
                        });

                    b.Navigation("Company")
                        .IsRequired();

                    b.Navigation("MotivationVideo");

                    b.Navigation("Position")
                        .IsRequired();
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateTestsAggregate.CandidateTests", b =>
                {
                    b.OwnsOne("Candidates.Domain.Aggregates.CandidateTestsAggregate.ValueObjects.Raport", "LgiGeneralFeedback", b1 =>
                        {
                            b1.Property<Guid>("CandidateTestsId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("InstanceId")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("Url")
                                .IsRequired()
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateTestsId");

                            b1.ToTable("CandidateTests", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateTestsId");
                        });

                    b.OwnsOne("Candidates.Domain.Aggregates.CandidateTestsAggregate.ValueObjects.Raport", "PapiDynamicWheel", b1 =>
                        {
                            b1.Property<Guid>("CandidateTestsId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("InstanceId")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("Url")
                                .IsRequired()
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateTestsId");

                            b1.ToTable("CandidateTests", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateTestsId");
                        });

                    b.OwnsOne("Candidates.Domain.Aggregates.CandidateTestsAggregate.ValueObjects.Raport", "PapiGeneralFeedback", b1 =>
                        {
                            b1.Property<Guid>("CandidateTestsId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("InstanceId")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("Url")
                                .IsRequired()
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateTestsId");

                            b1.ToTable("CandidateTests", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateTestsId");
                        });

                    b.OwnsOne("Candidates.Domain.Aggregates.CandidateTestsAggregate.ValueObjects.Raport", "PapiGeneralWheel", b1 =>
                        {
                            b1.Property<Guid>("CandidateTestsId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<string>("InstanceId")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("Url")
                                .IsRequired()
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateTestsId");

                            b1.ToTable("CandidateTests", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateTestsId");
                        });

                    b.OwnsOne("Candidates.Domain.Aggregates.CandidateTestsAggregate.ValueObjects.LogicalAssessment", "LogicalAssessment", b1 =>
                        {
                            b1.Property<Guid>("CandidateTestsId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<DateTimeOffset?>("CompletedAt")
                                .HasColumnType("datetimeoffset");

                            b1.Property<string>("PackageInstanceId")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("PackageTypeId")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<DateTimeOffset?>("StartedAt")
                                .HasColumnType("datetimeoffset");

                            b1.Property<string>("Status")
                                .IsRequired()
                                .HasMaxLength(64)
                                .HasColumnType("nvarchar(64)");

                            b1.Property<string>("Url")
                                .IsRequired()
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateTestsId");

                            b1.ToTable("CandidateTests", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateTestsId");

                            b1.OwnsOne("Candidates.Domain.Aggregates.CandidateTestsAggregate.ValueObjects.LogicScores", "Scores", b2 =>
                                {
                                    b2.Property<Guid>("LogicalAssessmentCandidateTestsId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<decimal>("Abstract")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("Accuracy")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("Numerical")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("Speed")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("Total")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("Verbal")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.HasKey("LogicalAssessmentCandidateTestsId");

                                    b2.ToTable("CandidateTests", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("LogicalAssessmentCandidateTestsId");
                                });

                            b1.Navigation("Scores");
                        });

                    b.OwnsOne("Candidates.Domain.Aggregates.CandidateTestsAggregate.ValueObjects.PersonalityAssessment", "PersonalityAssessment", b1 =>
                        {
                            b1.Property<Guid>("CandidateTestsId")
                                .HasColumnType("uniqueidentifier");

                            b1.Property<DateTimeOffset?>("CompletedAt")
                                .HasColumnType("datetimeoffset");

                            b1.Property<string>("PackageInstanceId")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<string>("PackageTypeId")
                                .IsRequired()
                                .HasMaxLength(256)
                                .HasColumnType("nvarchar(256)");

                            b1.Property<DateTimeOffset?>("StartedAt")
                                .HasColumnType("datetimeoffset");

                            b1.Property<string>("Status")
                                .IsRequired()
                                .HasMaxLength(64)
                                .HasColumnType("nvarchar(64)");

                            b1.Property<string>("Url")
                                .IsRequired()
                                .HasMaxLength(2000)
                                .HasColumnType("nvarchar(2000)");

                            b1.HasKey("CandidateTestsId");

                            b1.ToTable("CandidateTests", "candidate");

                            b1.WithOwner()
                                .HasForeignKey("CandidateTestsId");

                            b1.OwnsOne("Candidates.Domain.Aggregates.CandidateTestsAggregate.ValueObjects.PersonalityScores", "Scores", b2 =>
                                {
                                    b2.Property<Guid>("PersonalityAssessmentCandidateTestsId")
                                        .HasColumnType("uniqueidentifier");

                                    b2.Property<decimal>("A1")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("A2")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("AQ")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("R1")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("R2")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("S1")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("S2")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("SD")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("W1")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("W2")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("Y1")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.Property<decimal>("Y2")
                                        .HasColumnType("Decimal(18,2)");

                                    b2.HasKey("PersonalityAssessmentCandidateTestsId");

                                    b2.ToTable("CandidateTests", "candidate");

                                    b2.WithOwner()
                                        .HasForeignKey("PersonalityAssessmentCandidateTestsId");
                                });

                            b1.Navigation("Scores");
                        });

                    b.Navigation("LgiGeneralFeedback");

                    b.Navigation("LogicalAssessment");

                    b.Navigation("PapiDynamicWheel");

                    b.Navigation("PapiGeneralFeedback");

                    b.Navigation("PapiGeneralWheel");

                    b.Navigation("PersonalityAssessment");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Candidate", b =>
                {
                    b.Navigation("ActivityStatuses");

                    b.Navigation("Courses");

                    b.Navigation("DesiredSkills");

                    b.Navigation("Educations");

                    b.Navigation("Hobbies");

                    b.Navigation("Industries");

                    b.Navigation("Languages");

                    b.Navigation("Skills");

                    b.Navigation("WorkExperiences");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateAggregate.Entities.CandidateWorkExperience", b =>
                {
                    b.Navigation("Skills");
                });

            modelBuilder.Entity("Candidates.Domain.Aggregates.CandidateJobsAggregate.CandidateJobs", b =>
                {
                    b.Navigation("AppliedInJobs");

                    b.Navigation("ArchivedInJobs");

                    b.Navigation("SelectedInJobs");
                });
#pragma warning restore 612, 618
        }
    }
}
